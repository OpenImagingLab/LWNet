...
class StageII(nn.Module):
    def __init__(self):
        ...
        self.linears = nn.ModuleList([nn.Linear(1,1) for _ in range(10)])
        self.fc2 = zw()
        self.fc3 = FT()
    ...
    def forward(self, x,limit):
        ...
        if torch.rand(1)<=0.9521:
            ...
            out4 = torch.tanh(self.linears[0](out1[1].repeat(1, 1))) * out1[1] + self.linears[1](out1[1].repeat(1, 1))

        if torch.rand(1) <= 0.9181:
            out9 = torch.tanh(self.linears[2](out4)) * out4
        else:
            out9 = torch.tanh(self.linears[2](out4)) * out4 + self.linears[3](out4) * out4

        if torch.rand(1) <= 0.9687:
            out5 = torch.tanh(self.linears[4](out1[1].repeat(1, 1))) * out1[1]
        else:
            out5 = torch.tanh(self.linears[4](out1[1].repeat(1, 1))) * out1[1] + self.linears[5](out1[1].repeat(1, 1))* out1[1]

        if torch.rand(1) <= 0.9370:
            out7 = torch.tanh(self.linears[6](out1[3].repeat(1, 1))) * out1[3]
        else:
            out7 = torch.tanh(self.linears[6](out1[3].repeat(1, 1))) * out1[3] + self.linears[7](out1[3].repeat(1, 1))* out1[3]

        if torch.rand(1) <= 0.8582:
            out12 = torch.tanh(self.linears[8](out1[4].repeat(1, 1))) * out1[4]
        else:
            out12 = torch.tanh(self.linears[8](out1[4].repeat(1, 1))) * out1[4] + self.linears[9](out1[4].repeat(1, 1)) * out1[4]
